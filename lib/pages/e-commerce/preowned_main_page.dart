...
class _PreownedMainPageState extends State<PreownedMainPage> {
  int _currentIndex = 0;
  final List<String> categories = ['All', 'Electronics', 'Home Appliances', 'Furniture', 'Vehicles', 'Baby and Kids', 'Musical Instruments', 'Books & Stationery'];
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('preowned_products').get();
      List<Map<String, dynamic>> products = snapshot.docs.map((doc) {
        ...
        return {
          ...
          'seller': data['seller'] ?? '',
          'imageUrl1': data['imageUrl1'] ?? '', // This will be picked up by the widget logic
           ...data,
        };
      }).toList();
  ...
  void _applyFilters() {
    ...
    if (selectedCategory != 'All') results = results.where((p) => p['category'] == selectedCategory).toList();
    ...
    if (selectedFilter != 'All') {
       results.sort((a, b) {
        int cmp = 0;
        if (selectedFilter == 'Price') cmp = (a['price'] as double).compareTo(b['price'] as double);
        else if (selectedFilter == 'Name') cmp = (a['name'] ?? '').compareTo(b['name'] ?? '');
        ...
        children: [
              ...
              child: Column(
                children: [
                  Padding(
                    ...
                    child: Row(
                      children: [
                        IconButton(icon: const Icon(Icons.arrow_back, size: 24), onPressed: () => Navigator.pop(context)),
                        const Expanded(child: Text('Marketplace', style: TextStyle(fontFamily: 'Manrope', fontSize: 22, fontWeight: FontWeight.bold))),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Padding(
                          ...
                          child: ChoiceChip(
                            label: Text(cat),
                            selected: isSelected,
                            onSelected: (_) => setState(() { selectedCategory = cat; _applyFilters(); }),
                            backgroundColor: Colors.white,
                            selectedColor: const Color(0xFF388E3C),
                            labelStyle: TextStyle(color: isSelected ? Colors.white : Colors.black87, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal),
                            side: BorderSide(color: isSelected ? const Color(0xFF388E3C) : Colors.grey.shade300),
                          ),
                        );
                      },
                    ),
          ...
          if (searchQuery.isNotEmpty || selectedCategory != 'All')
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.filter_list, size: 16, color: Color(0xFF388E3C)),
                  const SizedBox(width: 8),
                  Expanded(child: Text(selectedCategory != 'All' ? 'Category: $selectedCategory' : 'Search results', style: const TextStyle(fontSize: 13, color: Color(0xFF388E3C), fontWeight: FontWeight.bold))),
                  TextButton(onPressed: _clearSearch, child: const Text('Clear', style: TextStyle(fontSize: 12, color: Color(0xFF388E3C), fontWeight: FontWeight.bold))),
                ],
              ),
            ),
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  ...
                  FilterChipButton(label: 'Name', isSelected: selectedFilter == 'Name', onTap: () => setState(() { selectedFilter = 'Name'; _applyFilters(); })),
                  if (selectedFilter != 'All')
            ...
            child: RefreshIndicator(
              ...
              child: isLoading
                  ...
                  : filteredProducts.isEmpty
                    ? Center(child: Text('No items found', style: TextStyle(fontSize: 18, color: Colors.grey.shade700)))
                    : GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return UniversalProductCard(
                            ...
                            isPreowned: true,   // Triggers Preowned logic (Location, Route)
                          );
                        },
                      ),
            ),
