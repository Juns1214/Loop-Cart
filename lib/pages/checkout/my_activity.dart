...
class _MyActivityPageState extends State<MyActivityPage>
  ...
  Future<void> _recalculateProductRating(String productId, bool isPreowned) async {
  try {
    ...
    print('🔄 Recalculating rating for product: $productId');
    ...
    if (reviewsSnapshot.docs.isEmpty) {
      print('⚠️ No reviews found for product: $productId');
      return;
    }
    ...
    for (var doc in reviewsSnapshot.docs) {
      int rating = (doc.data()['rating'] ?? 0) as int;
      sumRatings += rating;
    }

    double averageRating = sumRatings / totalReviews;

    print('📊 Calculated: $totalReviews reviews, average = ${averageRating.toStringAsFixed(2)}');
    ...
    if (productQuery.docs.isEmpty) {
      print('⚠️ Product not found in $productCollection: $productId');
      return;
    }

    // 4. Update product with new rating and total reviews
    ...
    print('✅ Updated $productCollection rating: ${averageRating.toStringAsFixed(2)} ($totalReviews reviews)');
  } catch (e) {
  ...
  Future<void> _showItemReviewDialog({
    ...
    await showDialog(
      context: context,
      // 1. FIX: Rename setState to setDialogState to avoid shadowing the parent's setState
                      ...
                      Row(
                        ...
                        children: List.generate(5, (index) {
                          return IconButton(
                            ...
                            onPressed: () {
                              // 2. FIX: Use setDialogState to update stars inside dialog
                              setDialogState(() {
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  ...
                  String collectionName = isPreowned
                      ? 'preowned_reviews'
                      : 'reviews';
                  ...
                  // 3. FIX: Update local data first
                  await _loadPurchaseHistory();

                  // 4. FIX: Check if widget is still mounted before using context
                  if (!context.mounted) return;

                  // 5. FIX: Show SnackBar BEFORE popping the dialog
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                  ...
                  // 6. FIX: Pop the dialog AFTER showing the SnackBar
                  Navigator.pop(context);

                  // 7. FIX: Update the PARENT page UI.
                  // Because we renamed the inner builder's state to 'setDialogState',
                  // calling 'setState' here correctly refers to the Page's setState.
                  setState(() {});
                } catch (e) {
                  print('Error submitting review: $e');
                  // Check mounted again for error handling
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
  ...
  Future<void> _showSubmittedReviewDialog(
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(Icons.check_circle, color: Color(0xFF388E3C), size: 28),
            SizedBox(width: 12),
            Text(
              'Your Review',
                  ...
                  if (isReceived && !allItemsReviewed) ...[
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          reviewedCount == 0 ? 'Review Items' : 'Review More',
                        ),
                        style: ElevatedButton.styleFrom(
